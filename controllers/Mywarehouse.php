...
//	}
    ...
    $query = $this->db->get('warehouse_sku');

    foreach ($query->result_array() as $row) {
        $this->db->where("parent", $row['wsid']);
        $calculate_rows = $this->db->get("warehouse_sku")->num_rows();
        if($row['parent'] == 0 )
        {
            $icon = 'folder';
        }else
        ...
        if($calculate_rows != 0){
            $data_final[] = array(
                "id" => $row['wsid'],
                "value" => $row['title']." (<font color='green'><b>$calculate_rows</b></font>)",
                "parent_id" => $row['parent'],
                'listing_title' => $row['wsid'],
                'title' => $row['title']." (<font color='green'><b>$calculate_rows</b></font>)",
                "icon" => "$icon"
            );
        }else
        {
    ...
    $this->mysmarty->assign('skulist', $data);
...
function open($page = 1, $cat = 1)
	{	
        ...
        //LOAD DATA TO HANDSONTABLE
        foreach($load['results'] as $list)
        {
            $total_cost = $load['per'][$list['waid']]['accounting']['cost'] + $load['per'][$list['waid']]['accounting']['shipped_inbound'] + $load['per'][$list['waid']]['accounting']['shipped_actual'] + $load['per'][$list['waid']]['accounting']['sellingfee'] + $load['per'][$list['waid']]['auctionshipping'] + $load['per'][$list['waid']]['auctionexpenses'];
            $cost = $load['per'][$list['waid']]['accounting']['cost'];
            $revenue = $load['per'][$list['waid']]['accounting']['paid'];
            $gross = ($revenue - $total_cost);
            @$percent_gross = round((($revenue + $load['per'][$list['waid']]['accounting']['shipped_actual'] - $total_cost) / $total_cost) * 100)."%";
            $sold_cost = $load['per'][$list['waid']]['accounting_sold']['cost'];
            @$total_cost_sold = $load['per'][$list['waid']]['accounting_sold']['cost'] + $load['per'][$list['waid']]['accounting_sold']['shipped_inbound'] + $load['per'][$list['waid']]['accounting_sold']['shipped_actual'] + $load['per'][$list['waid']]['accounting_sold']['sellingfee'];
            @$sold_margin = round((($revenue + $load['per'][$list['waid']]['accounting_sold']['shipped'] - $total_cost_sold) / $total_cost_sold) * 100)."%";

            //print_r($load['per'][$list['waid']]['statuses']);
            $auction_hot[] = array(
                "waid" => $list['waid'],
                "wtitle" => $list['wtitle'],
                "wdate" => date( 'm/d/Y H:i:s', strtotime($list['wdate'])),
                "cost" => $cost,
                "revenue" => $revenue,
                ...
                "not_sold_cost" => $load['per'][$list['waid']]['accounting_notsold']['cost'],
                "total_qty" => $load['per'][$list['waid']]['cnt'],
                "sold_qty" => $load['per'][$list['waid']]['cnt_sold'],
                "repairing_qty" => $load['per'][$list['waid']]['statuses']['Repairing'],
                "nottested_qty" => $load['per'][$list['waid']]['statuses']['Not Tested'],
                "readytosell_qty" => $load['per'][$list['waid']]['statuses']['Ready to Sell'],
                "partsneeded_qty" => $load['per'][$list['waid']]['statuses']['Parts Needed'],
                "with_location" => $load['per'][$list['waid']]['location']['Y'],
            );
        ...
        $this->mysmarty->assign("auctions_hot", json_encode($auction_hot));
		//printcool ($load);
