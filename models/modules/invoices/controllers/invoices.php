...
class Invoices extends Admin_Controller {
    ...
    public function receive_payment($invoice_id, $client_id) {
        $data['client_data'] = array(
            'client_id' => $client_id,
            'invoice_id' => $invoice_id
        );

        $this->load->view("receive_card_data", $data);
    ...
    public function view($invoice_id) {
        ...
        //GET PAYMENT LOG FROM WEBSITE
        $this->db->where("invoice_id", $invoice_id);
        $this->db->order_by("id", "DESC");
        $payment_data = $this->db->get("ip_authorizenet_log")->result_object();
    ...
    public function delete($invoice_id) {
        ...
        if ($invoice_status == 1 || $this->config->item('enable_invoice_deletion') === true) {
            // Delete the amendment from 365laptoprepair
