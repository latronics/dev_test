...
class Myebaydim extends Controller {
    ...
    function StartReturn($orderid, $ordertype, $notfirst = 0) {
        ...
        if ($o) {
            ...
            $this->mysmarty->assign('salebcns', $this->Myseller_model->getSales(array((int) $orderid), (int) $ordertype, true, true));
    ...
    function SetReturned($orderid, $ordertype, $notfirst = 0) {
        ...
        if ($o) {
            if ($notfirst > 0) {
                ...
                $v[] = $o;
                unset($v['return_id']);
                $this->db->update('returns', array('vlog' => serialize($v)), array('vid' => $o['return_id']));
            }
            ...
            if ($notfirst > 0) {
                ...
                $this->db->insert('returns', array('orderid' => (int) $orderid, 'channel' => (int) $ordertype, 'vdate' => CurrentTime(), 'adminid' => (int) $this->session->userdata['admin_id']));
    ...
    function SetReceived($orderid, $ordertype, $notfirst = 0, $onlybcn = false) {
        ...
        if ($o || $onlybcn) {
            ...
            if ((int) $insert['return_id'] == 0) {
                GoMail(array('msg_title' => 'return_id 0', 'msg_body' => printcool($o, true) . printcool((int) $orderid, true) . printcool((int) $ordertype, true), 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
                $retake = $this->Myebay_model->getOrderReturn((int) $orderid, (int) $ordertype);
                $insert['return_id'] = $retake['return_id'];

                if ((int) $insert['return_id'] == 0)
                    GoMail(array('msg_title' => 'return_id 0 II', 'msg_body' => printcool($retake, true) . printcool((int) $orderid, true) . printcool((int) $ordertype, true), 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            }
            ...
            if ($bcns) {
                foreach ($bcns as $ka => $b) {
                    ...
                    if (!$onlybcn || ($onlybcn && $onlybcn == $b['wid']) && isset($binsert)) {
                        $this->db->update('warehouse', $binsert, array('wid' => (int) $b['wid']));
    ...
    function SetRefunded($orderid, $ordertype, $notfirst = 0, $onlybcn = false) {
        ...
        if ($o || $onlybcn) {
            ...
            if ((int) $insert['return_id'] == 0) {
                GoMail(array('msg_title' => 'refund return_id 0', 'msg_body' => printcool($o, true) . printcool((int) $orderid, true) . printcool((int) $ordertype, true), 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
                $retake = $this->Myebay_model->getOrderReturn((int) $orderid, (int) $ordertype);
                $insert['return_id'] = $retake['return_id'];

                if ((int) $insert['return_id'] == 0)
                    GoMail(array('msg_title' => 'refund return_id 0 II', 'msg_body' => printcool($retake, true) . printcool((int) $orderid, true) . printcool((int) $ordertype, true), 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            }
    ...
    function ChangeOOS($eid, $oos) {
        if ((int) $eid > 0 && ((int) $oos == 1 || (int) $oos == 0)) {
            $this->db->update('ebay', array('ooskeepalive' => (int) $oos), array('e_id' => (int) $eid));
            if ((int) $oos == 1)
    ...
    function _refreshlisting($eid) {
        ...
        if ($q->num_rows() > 0) {
            ...
            echo json_encode(array('a' => $this->mysmarty->fetch('myebay/myebay_show_loop1.html'), 'b' => $this->mysmarty->fetch('myebay/myebay_show_loop2.html'), 'c' => $this->mysmarty->fetch('myebay/myebay_show_loop3.html'), 'd' => $this->mysmarty->fetch('myebay/myebay_show_loop4.html')));
        }
    }
    ...
    function SaveOrderNotes($id = '', $otype = '') {
        ...
        $id = (int) $id;
        $this->db->update($place, $arr, array($dbf => $id));
        echo ($value);
    }
    ...
    function SaveOrderAccounted($id = '', $otype = '') {
        ...
        $this->db->update($place, array('accounted' => (int) $value), array($dbf => (int) $id));
        echo ((int) $value);
    }
        ...
        foreach ($q->result_array() as $k => $v) {
            ...
            if (isset($_POST['items'][$v['e_id']])) {
                if ($v['weight_lbs'] != (float) $_POST['items'][$v['e_id']]['lbs'] || $v['weight_oz'] != (float) $_POST['items'][$v['e_id']]['oz']) {
                    $this->db->update('ebay', array('weight_lbs' => (float) $_POST['items'][$v['e_id']]['lbs'], 'weight_oz' => (float) $_POST['items'][$v['e_id']]['oz'], 'weight_kg' => lbsoz2kg((float) $_POST['items'][$v['e_id']]['lbs'], (float) $_POST['items'][$v['e_id']]['oz'])), array('e_id' => (int) $_POST['items'][$v['e_id']]['eid']));
    ...
    function gtset($page = 1) {
        ...
        foreach ($q->result_array() as $k => $v) {
            $list[$k] = $v;
            if (isset($_POST['items'][$v['e_id']])) {
                $this->db->update('ebay', array('gtaxonomy' => $_POST['items'][$v['e_id']]['gtaxonomy']), array('e_id' => (int) $_POST['items'][$v['e_id']]['eid']));
    ...
    function getrevised() {
        ...
        if ($this->query->num_rows() > 0) {
            foreach ($this->query->result_array() as $k => $v) {
                $this->db->update('ebay', array('autorev' => 0), array('e_id' => (int) $v['e_id']));
            }
        }
    }
    ...
    function _DoCounting()
    {
        ...
        $listingcount[0] = array("id" => "{$site_url}Myebay/viewport/clear",
            "parent" => "0",
            "text" => "Listings: (" . $this->db->count_all_results('ebay') . ")");
        ...
        $listingcount[1] = array("id" => "{$site_url}Myebay/viewport/ActiveWebsite",
            "parent" => "0",
            "text" => "Website (" . $this->db->count_all_results('ebay') . ")");
        ...
        $listingcount[5] = array("id" => "{$site_url}Myebay/viewport/ActiveEbayLocal",
            "parent" => "0",
            "text" => "eBay Local (" . $linked . ")");
        ...
        //$this->db->where('ebay_submitted', NULL);
         ...
         $this->db->where('ebay_id !=', 0);
          $this->db->where('ebended', NULL);
          $this->db->where('qn_ch2 != ebayquantity');

        $listingcount[9] = array( "id" => "{$site_url}Myebay/viewport/ActiveEbayLocalBCNQTYMisMatch",
            "parent" => "0",
            "text" => "Channel/eBayQ Mismatch (".$this->db->count_all_results('ebay').")");
        ...
        $listingcount[10] = array( "id" => "{$site_url}Myebay/viewport/ActiveEbayActiveBCNQTYMisMatch",
            "parent" => "0",
            "text" => "ActiveBCN/eBayQ Mismatch (".$this->db->count_all_results('ebay').")");
        ...
        $listingcount[13] = array( "id" => "{$site_url}Myebay/viewport/ActiveEbayAssignedMisMatch",
            "parent" => "0",
            "text" => "Assigned/eBayQ Mismatch (".$this->db->count_all_results('ebay').")");
        ...
        $listingcount[11] = array( "id" => "{$site_url}Myebay/viewport/OutOfStock",
            "parent" => "0",
            "text" => "OOS Needs Attention (".$this->db->count_all_results('ebay').")");
        ...
        $listingcount[12] = array( "id" => "{$site_url}Myebay/viewport/OosKeepAlive",
            "parent" => "0",
            "text" => "OOS Keep Alive (".$this->db->count_all_results('ebay').")");
        ...
        $listingcount[8] = array( "id" => "{$site_url}Myebay/viewport/ActiveEbayLocalBCNQTYMatch",
            "parent" => "0",
            "text" => "Channel/eBayQ Match (".$this->db->count_all_results('ebay').")");
        $this->mysmarty->assign('quantbcn', $this->db->count_all_results('ebay'));
        $this->db->where('etype', 'a');

        $listingcount[2] = array("id" => "{$site_url}Myebay/nav/2",
            "parent" => "0",
            "text" => "eBay LIVE (" . $this->db->count_all_results('ebay_live') . ")");
        ...
        $listingcount[15] = array( "id" => "{$site_url}Myebay/viewport/ActiveEbayLocalGhost",
            "parent" => "0",
            "text" => "Has Ghosts (".$this->db->count_all_results('ebay').")");
        ...
        $listingcount[14] = array( "id" => "{$site_url}Myebay/viewport/ActiveEbayLocalGreens",
            "parent" => "0",
            "text" => "Has Greens (".$q->num_rows().")");
        ...
        $listingcount[17] = array( "id" => "{$site_url}Myebay/viewport/Audited",
            "parent" => "0",
            "text" => "Audited Listings (".$eaudited.")");
        ...
        $listingcount[21] = array( "id" => "{$site_url}Myebay/viewport/Autopilot",
            "parent" => "0",
            "text" => "Autopiloted (".$query->num_rows().")");
        ...
        $listingcount[22] = array( "id" => "{$site_url}Myebay/viewport/Expiredpilot",
            "parent" => "0",
            "text" => "Inactive Autopilot (".$query->num_rows().")");
        $this->mysmarty->assign('inactiveautopilot', $query->num_rows());

        $query = $this->db->query('SELECT distinct e_id FROM competitor_rules');

        $listingcount[23] = array( "id" => "{$site_url}Myebay/viewport/Competitor",
            "parent" => "0",
            "text" => "Competitor (".$query->num_rows().")");
        ...
        $listingcount[24] = array( "id" => "{$site_url}Myebay/viewport/Dispose",
            "parent" => "0",
            "text" => "Dispose (".$query->num_rows().")");
        ...
        $listingcount[25] = array( "id" => "{$site_url}Myebay/viewport/Ebayspec",
            "parent" => "0",
            "text" => "ItemSpec (".$query->num_rows().")");
        ...
        $listingcount[19] = array( "id" => "{$site_url}Myebay/viewport/BcnsAudited",
            "parent" => "0",
            "text" => "Audited Bcns (".$bcnsaud.")");
        ...
        $listingcount[20] = array( "id" => "{$site_url}Myebay/viewport/BcnsNotAudited",
            "parent" => "0",
            "text" => "Not Audited Bcns (".($q->num_rows() - $bcnsaud).")");
        ...
        $listingcount[18] = array( "id" => "{$site_url}Myebay/viewport/NotAudited",
            "parent" => "0",
            "text" => "Not Audited Listings (".($linked - $eaudited).")");
        ...
        $listingcount[4] = array("id" => "{$site_url}Myebay/nav/4",
            "parent" => "0",
            "text" => "Not Linked (" . $activenotlinked . ")");

        $this->mysmarty->assign('activenotlinked', $activenotlinked);

        $listingcount[3] = array("id" => "{$site_url}Myebay/nav/3",
            "parent" => "0",
            "text" => "Linked (" . $activelinked . ")");
        ...
        $listingcount[7] = array( "id" => "{$site_url}Myebay/viewport/MisMatched",
            "parent" => "0",
            "text" => "with Mismatched BCNs (".$q->num_rows().")");
        ...
        $listingcount[16] = array( "id" => "{$site_url}Myebay/ReviseLog/1/1",
            "parent" => "0",
            "text" => "Severe ReviseLog (".$q->num_rows().")");
        ...
        if (isset($lst) && count($lst) > 0) {
            ...
            $listingcount[6] = array( "id" => "$site_url}Myebay/viewport/NoTransBCNs",
                "parent" => "0",
                "text" => "with Sale missing BCN Assign (".$q->num_rows().")");
            $this->mysmarty->assign('notransbcns', $q->num_rows());
        } else {
            $listingcount[6] = array( "id" => "$site_url}Myebay/viewport/NoTransBCNs",
                "parent" => "0",
                "text" => "with Sale missing BCN Assign (0)");
            $this->mysmarty->assign('notransbcns', 0);
        }

        $this->mysmarty->assign('floatnav1', str_replace('\"', '', json_encode($categories_json)));
    ...
    function viewport($view = 'clear', $page = 1) {
        ...
        switch ($view) {
            /* 	   • Active
              ...
              → eBay Local (312)
              → BCN/QTY Match (31)
              → BCN/QTY Mismatch (281)
              → Has Ghosts (219)
              • Inactive
              → Website (13976)
              → eBay Local (4151)
              → BCN/QTY Match (3939)
              ;      → BCN/QTY Mismatch (212)
              → Has Ghosts (0)
             */
            case "ActiveWebsite":
    ...
    function ListItems($page = 1, $qtt_show = 30, $searched = false) {
...
//printcool ($_POST['zero']);
        ...
        $data = $this->Myebay_model->ListItems($string, $where, $ended, $zero, $mm, $bcnmm, $sitesell, $page, $qtt_show);
        ...
        $this->mysmarty->assign('json_categories', $this->loadcat());
        $this->mysmarty->assign('categories', $this->db->get("ebay_cat_xml")->result_array());
        ...
        $config['total_rows'] = $data['count']['all'] / $qtt_show;
        $config['per_page'] = 1;
        $config['base_url'] = base_url() . 'Myebay/ListItems';
        ...
        $this->mysmarty->assign("total_rows", $data['count']['all']);
        $this->mysmarty->assign("total_pages", round($data['count']['all'] / $qtt_show));
    ...
    function savecat() {
        ...
        $this->db->update("ebay_cat_xml", $category_array);
    }

    function catsearch() {
        $cat_id = $this->input->post("cat_id");

        if (($cat_id != null) && (!array($cat_id))) {
        ...
        $categories = $this->db->get("ebay_cat_xml")->result_object();
        echo json_encode($categories[0]);
    }

    function loadcat() {
        $sc = $this->db->get("ebay_cat_xml")->result_object();

        foreach ($sc as $categories) {
        ...
        return str_replace('\"', '', json_encode($categories_json));
    }

    function tablize($eid = 0) {
        $this->load->model('Myseller_model');
        $this->Myseller_model->getBase(array((int) $eid));
    ...
    function ReplaceEbayId($eid = '', $parent = '') {
        ...
        if ((int) $eid != 0 && (int) $parent != 0)
            $this->db->update('ebay', array('ebay_id' => (int) $parent, 'ebended' => NULL), array('e_id' => $eid));
    ...
    function RevertAction($alid = 0, $page = 1) {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($ra['field'] == 'sn')
                $this->db->update('ebay_transactions', array($ra['field'] => $ra['datafrom']), array('rec' => (int) $ra['trans_id']));
            elseif ($ra['field'] == 'asc')
                $this->db->update('ebay_transactions', array($ra['field'] => $ra['datafrom']), array('rec' => (int) $ra['trans_id']));
            /* elseif ()
              {
                ...
                $this->db->update('ebay', array($ra['field'] => $ra['datafrom']), array('e_id' => (int) $ra['e_id']));
...
';
    ...
    function Edit($itemid = '', $catID = 0, $merge = false, $called = false) {
        ...
        //LOAD CATEGORIES TAB INFO
        //echo $this->config->config['ebaypath'].'/cats.txt';
        $this->load->helper('directory');
        $this->load->helper('file');
        ...
        if (isset($store->Store->CustomCategories->CustomCategory)) {
            foreach ($store->Store->CustomCategories->CustomCategory as $s) {
                ...
                $treearray['open'] = 'true';
                //$treearray['icon']= 'folder';

                /* Function showcategoryListings is in -> views\mysku\myproducts_categories.html */

                $treearray['data'][] = array(
                    'id' => $a['CategoryID'],
                    'value' => '<a id="mylin0k" onclick="showcategoryListings(' . $a['CategoryID'] . ',\'' . $a['Name'] . '\')">' . substr($a['Name'], 0, 25) . '</a>',
                    'open' => false,
                    'listing_title' => 'Category ' . $a['CategoryID'],
                    'title' => $a['Name'],
                    'data' => $this->LoadProductsInCategory($a['CategoryID'])
                );

                $catIdarray[] = $a['CategoryID'];
            }
        }
...
//				$this->inputdata['e_desc'] = $this->ieditor->CreateHtml();	
                ...
                if (strlen($this->displays['eBay_specs']) > 10) {
                    ...
                    $this->db->where("list_id", $itemid);
                    $item_spec = $this->db->get("ebay_specs")->result_object();

                    $this->mysmarty->assign('displays', $item_spec);
                    //$this->mysmarty->assign('displays', $this->displays);
                ...
                /* if ($called) $this->checkexists =  $this->Myebay_model->CheckSefExists($this->db_data['e_sef']);
                  else */$this->checkexists = $this->Myebay_model->CheckSefExists($this->db_data['e_sef'], $this->id);
                if ($this->checkexists)
                    ...
                    if ($f->num_rows() > 0) {
                        ...
                        foreach ($fr as $fl) {
                            if ($fl['title'] != $this->db_data['e_title']) {
                                ...
                                $this->db->update('warehouse', array('title' => $this->db_data['e_title']), array('wid' => $fl['wid']));
                            }
                        }
                    }
                }
...
//    }
    ...
    function UpdateQuantityFromActive($elid = 0) {
        ...
        if ($query->num_rows() > 0) {
            $r = $query->row_array();
            $this->db->update('ebay', array('qn_1' => $r['ebavq'], 'ebayquantity' => $r['ebtq']), array('e_id' => (int) $r['eid']));
            $this->db->update('ebay_live', array('lq' => $r['ebavq'], 'lebq' => $r['ebtq']), array('el_id' => (int) $elid));
    ...
    function DoWaterMark($id, $place = 1, $attempt = 1, $backtoedit = false) {
        ...
        if ($img) {
            if (!copy($this->config->config['paths']['imgebay'] . '/' . idpath((int) $id) . $img, $this->config->config['paths']['imgebay'] . '/' . idpath((int) $id) . 'Original_' . $img)) {
                ...
                GoMail(array('msg_title' => 'failed to copy Original_file (attempt ' . $attempt . ')', 'msg_body' => (int) $id . ', ' . $place . ', ' . $img . ' @' . CurrentTime(), 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            ...
            if (!copy($this->config->config['paths']['imgebay'] . '/' . idpath((int) $id) . $img, $this->config->config['paths']['imgebay'] . '/' . idpath((int) $id) . 'Ebay_' . $img)) {
                ...
                GoMail(array('msg_title' => 'failed to copy Ebay_file (attempt ' . $attempt . ')', 'msg_body' => (int) $id . ', ' . $place . ', ' . $img . ' @' . CurrentTime(), 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
        ...
        if ($q->num_rows() > 0) {
            ...
            foreach ($this->productimages as $value) {
                if ($similar['e_img' . $value] != '') {
                    ...
                    $this->db->update('ebay', array('idpath' => str_replace('/', '', $idpath), 'e_img' . $value => str_replace($e['e_id'], $id, $similar['e_img' . $value])), array('e_id' => $id));
                }
            }
      ...
      <ShippingCostPaidByOption>Buyer</ShippingCostPaidByOption>
        ...
        <ShipToLocation>" . $s['ShipToLocation'] . "</ShipToLocation>
            ...
            if (isset($xmlvresp->Errors)) {
                ...
                $this->_recordsubmiterror(array('msg_title' => 'SUBMITTED ERRORS ' . (int) $id . ' @' . CurrentTime(), 'msg_body' => $estr, 'msg_date' => CurrentTime()));
...
/////////////////////
            ...
            if ($itemID > 0) {
                ...
                $this->db->update('ebay', array('ebay_submitted' => CurrentTime(), 'ebay_msubm' => mktime(), 'ebay_id' => $itemID, 'Ack' => $ack, 'link' => $linkBase . $itemID, 'InsertionF' => $ifee, 'ListingF' => $lfee, 'ebayquantity' => $quantity, 'unsubmited' => 1, 'ebended' => NULL), array('e_id' => (int) $id));
                ...
                $workdata = array('newvals' => array(
                        ...
                        array('name' => 'wherelisted',
                            'value' => 'eBay (' . $itemID . ')'
                        ),
                        array('name' => 'datelisted',
                            'value' => CurrentTime()
                        )
                    ),
                    'origin' => (int) $id,
            ...
            if (isset($this->resubmit) && $itemID > 0)
                $this->db->update('ebay', array('ebended' => NULL, 'submitlog' => 'Resubmited @ ' . CurrentTime() . ' by ' . $this->session->userdata['ownnames'] . ' - Previous submit: ' . $item['ebay_submitted'] . ' ID ' . $item['ebay_id'] . '<br>' . $item['submitlog']), array('e_id' => (int) $id));
...
-->
                    ...
                    if (count($aresponse['Errors']) > 0) {
                        foreach ($aresponse['Errors'] as $d) {
                            ...
                            if (isset($d['ErrorParameters']['Value'])) {
                                ...
                                GoMail(array('msg_title' => 'Submit Error for ' . (int) $id . ' @ ' . CurrentTime(), 'msg_body' => $d['LongMessage'] . '<br><br>' . $d['ErrorParameters']['Value'], 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
                                ...
                                GoMail(array('msg_title' => 'Submit Error for ' . (int) $id . ' @ ' . CurrentTime(), 'msg_body' => $d['LongMessage'], 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
  ...
  <ItemID>" . $item['ebay_id'] . "</ItemID>";
            ...
            GoMail(array('msg_title' => 'End listing step 1 - ' . (int) $id . ' @' . CurrentTime(), 'msg_body' => $requestXmlBodySTART . $requestXmlBody . $requestXmlBodyEND, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);

            $responseXml = $session->sendHttpRequest($requestXmlBodySTART . $requestXmlBody . $requestXmlBodyEND);
            if (stristr($responseXml, 'HTTP 404') || $responseXml == '')
                die('<P>Error sending request');
            GoMail(array('msg_title' => 'End listing step 2 - ' . (int) $id . ' @' . CurrentTime(), 'msg_body' => $responseXml, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            //Xml string is parsed and creates a DOM Document object
            $responseDoc = new DomDocument();
            $responseDoc->loadXML($responseXml);
            ...
            $sxml = simplexml_load_string($responseXml);
            $aresponse = $this->_XML2Array($sxml);
            GoMail(array('msg_title' => 'End listing step 3 - ' . (int) $id . ' @' . CurrentTime(), 'msg_body' => $aresponse, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            $errors = $responseDoc->getElementsByTagName('Errors');
            ...
            if ($errors->length > 0) {
                ...
                $longMsg = $errors->item(0)->getElementsByTagName('LongMessage');
                $severity = $errors->item(0)->getElementsByTagName('SeverityCode');
                GoMail(array('msg_title' => 'End listing step 4 - ' . (int) $id . ' @' . CurrentTime(), 'msg_body' => $longMsg, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
                log_message('error', 'SUBMIT STEP 11 ' . (int) $id . ' @' . CurrentTime());
            ...
            $ack = $acks->item(0)->nodeValue;
            GoMail(array('msg_title' => 'End listing step 5 - ' . (int) $id . ' @' . CurrentTime(), 'msg_body' => $ack, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            if ($ack == 'Success')
                $this->db->update('ebay', array('ebended' => CurrentTime(), 'endedreason' => $reason . ' by ' . $this->session->userdata['ownnames']), array('e_id' => (int) $id));
...
<ReviseItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">';
	...
	" . $imgsnippet;
        ...
        <ShipToLocation>" . $s['ShipToLocation'] . "</ShipToLocation>
            ...
            //GoMail(array ('msg_title' => 'REVISED '.(int)$id.' @'.CurrentTime(), 'msg_body' => $requestXmlBody, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
...
<ReviseItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">';
    ...
    <ItemID>" . $item['ebay_id'] . "</ItemID>";
                ...
                if ($errors->length > 0) {
                    ...
                    $this->_recordsubmiterror(array('msg_title' => 'REVISE DESCRIPTION ERRORS ' . (int) $r['e_id'] . ' @' . CurrentTime(), 'msg_body' => printcool(str_replace(">", "&gt;", str_replace("<", "&lt;", $longMsg->item(0)->nodeValue)), TRUE), 'msg_date' => CurrentTime()));
                    ...
                    if (isset($_POST['eid']))
                        exit($txtresp);
                    //GoMail(array ('msg_title' => 'REVISED DESCRIPTION '.(int)$id.' @'.CurrentTime(), 'msg_body' => $txtresp, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
                    //if ($save) $this->db->update('ebay', array('autorev' => 1, 'autorevtxt' => '@ '.CurrentTime()), array('e_id' => (int)$id));
                }
                $this->db->update('ebay', array('housekeeping' => 1), array('e_id' => $r['e_id']));
                printcool($r);
            }
        }
    }
...
<ReviseInventoryStatusRequest xmlns="urn:ebay:apis:eBLBaseComponents">';
            ...
            //GoMail(array ('msg_title' => 'INVENTORY UPDATED '.(int)$id.' @'.CurrentTime(), 'msg_body' => $requestXmlBody, 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            ...
            if ($errors->length > 0) {
                ...
                $this->db->update('ebay', array('ebayquantity' => $item['qn_ch1']), array('e_id' => (int) $id));
...
// KHIM change query to return different result set
    ...
    function Orders($page = 1, $perpage = 100, $filtertype = false, $filtersubtype = false) {
        ...
        /* if (isset($this->sorttype) && !isset($_POST['osrc']) && !isset($this->listingid))
          {
          switch ($this->sorttype)
          {
          case 24: //NEEDSATTENTION
          //?????????????
          if (isset($etids))
          {
          $where = 't`.`e_id` >= 0 AND (';
          foreach ($etids as $e) $where .= "`t`.`et_id` = ".$e.' OR ';
        ...
        if ($filtertype && !isset($_POST['osrc']) && !isset($this->listingid))
            switch ($filtertype) {
                //CHANNEL1 = EBAY
                //CHANNEL2 = WEB
                //CHANNEL4 = WARE
        ...
        if ($filtersubtype && !isset($_POST['osrc']) && !isset($this->listingid))
        ...
        if (!isset($_POST['osrc']) && !isset($this->listingid)) {
            if (isset($this->sorttype) && $this->sorttype == 2 && isset($this->spectype) && $this->spectype == 2)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/NotShipped');
            elseif (isset($this->sorttype) && $this->sorttype == 2)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/Ebay');
            elseif (isset($this->sorttype) && $this->sorttype == 23)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . 'EbayPendingPay');
            elseif (isset($this->sorttype) && $this->sorttype == 21)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/EbayNotPaid');
            elseif (isset($this->sorttype) && $this->sorttype == 25)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/EbayPartialRefund');
            elseif (isset($this->sorttype) && $this->sorttype == 24)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/NeedAttention');
            elseif (isset($this->sorttype) && $this->sorttype == 3)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/Site');
            elseif (isset($this->sorttype) && $this->sorttype == 10)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/Ware');
            elseif (isset($this->sorttype) && $this->sorttype == 4)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/Paid');
            elseif (isset($this->sorttype) && $this->sorttype == 9)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/NoPaid');
            elseif (isset($this->sorttype) && $this->sorttype == 5)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/Processed');
            elseif (isset($this->sorttype) && $this->sorttype == 6)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/NoProcessed');
            elseif (isset($this->sorttype) && $this->sorttype == 7)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/Asc');
            elseif (isset($this->sorttype) && $this->sorttype == 8)
                $this->mysmarty->assign("gotourl", 'Orders/' . (int) $page . '/' . (int) $perpage . '/NoAsc');
            else
                $this->mysmarty->assign("gotourl", 'Orders');
        }
        $this->session->set_userdata('sorttype', $this->sorttype);
        $this->session->set_userdata('sortstring', $this->sortstring);
...
');
        ...
        /* $sql = "SELECT  mkdt as timekey, et_id AS orderkey, 'ebay' as typekey  FROM ebay_transactions";
          $sql .= " UNION ALL ";
          $sql .= " SELECT timemk as timekey, woid AS orderkey , 'warehouse' as typekey FROM warehouse_orders";
          $sql .= " UNION ALL ";
          $sql .= " SELECT submittime as timekey,  oid AS orderkey, 'website' as typekey  FROM orders ";
          $sql .= " ORDER BY `timekey`  DESC limit ".$limit;
         */


        /*
          $sql = "SELECT  mkdt as timekey, et_id AS orderkey, 'ebay' as typekey,

          paidtime as field_timepaid,
          notpaid as field_notpaid,
          mark as field_mark,
          customcode as field_customcode,
          returned as field_returned,
          returned_refunded as field_refunded,
          sellingstatus as field_sellingstatus

          FROM ebay_transactions";

          $sql .= " UNION ALL ";

          $sql .= " SELECT timemk as timekey, woid AS orderkey , 'warehouse' as typekey,

          time as  field_timepaid,
          '' as field_notpaid,
          mark as field_mark,
          '' as field_customcode,
          returned as field_returned,
          ...
          $sql .= " UNION ALL ";
          $sql .= " SELECT submittime as timekey,  oid AS orderkey, 'website' as typekey,
          ...
          returned as field_returned,
          returned_refunded as field_refunded,
          '' as field_sellingstatus  FROM orders ";
          $sql .= " ORDER BY `timekey`  DESC limit ".$limit;
         */
        ...
        if ($channel[1]) {
            ...
            if (isset($this->listingid))
                $csql .= ' FROM (ebay_transactions e) WHERE e_id = "' . (int) $this->listingid . '" ';
            elseif (isset($_POST['osrc']))
                $csql .= " FROM (ebay_transactions e) WHERE et_id = '" . $this->input->post('osrc', TRUE) . "' OR  buyerid LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR buyeremail LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR notes LIKE '%" . $this->input->post('osrc', TRUE) . "%' ";
            elseif (isset($this->sorttype) && $this->sorttype == 24)
                $csql .= " FROM (ebay_transactions e) LEFT JOIN warehouse w ON e.et_id = w.sold_id WHERE (e.notpaid != 0 OR e.refunded != 0 OR e.returnnotif IS NOT NULL) AND w.vended > 0 AND w.deleted = 0 AND w.nr = 0 AND w.channel = 1 AND w.sold_id != 0";
            elseif ($timeframe)
                $csql .= " FROM (ebay_transactions e) WHERE mkdt <= " . $ofrom . " AND mkdt >= " . $oto . " ";
            else
                $csql .= " FROM (ebay_transactions e) ";
        ...
        if ($channel[4]) {
            ...
            //$csql .= " ";
            if (isset($this->listingid))
                $csql .= ' FROM (warehouse_orders o) LEFT JOIN warehouse w ON o.woid = w.sold_id WHERE w.vended > 0 AND w.deleted = 0 AND w.nr = 0 AND w.channel = 4 AND w.listingid = "' . (int) $this->listingid . '" ';
            elseif (isset($_POST['osrc']))
                $csql .= " FROM (warehouse_orders o) WHERE woid = '" . $this->input->post('osrc', TRUE) . "'  OR buyer LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR notes LIKE '%" . $this->input->post('osrc', TRUE) . "%' ";
            elseif ($timeframe)
                $csql .= " FROM (warehouse_orders o) WHERE timemk <= " . $ofrom . " AND timemk >= " . $oto . " ";
            $csql .= $psql[4];
        ...
        if ($channel[2]) {
            ...
            // $csql .=  "";
            if (isset($this->listingid))
                $csql .= ' FROM orders WHERE eids LIKE "%|' . (int) $this->listingid . '|%"';
            elseif (isset($_POST['osrc']))
                $csql .= " FROM orders WHERE oid = '" . $this->input->post('osrc', TRUE) . "' OR fname LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR lname LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR email LIKE '%" . $this->input->post('osrc', TRUE) . "%' ";
            elseif ($timeframe)
                $csql .= " FROM orders  WHERE submittime <= " . $ofrom . " AND submittime >= " . $oto . " ";
            $csql .= $psql[2];
        }
        ...
        if (isset($_POST['osrc']))
            ...
            $this->mysmarty->assign('osrc', false);
...
''	as	delivery
  ";
            ...
            if (isset($this->sorttype) && $this->sorttype == 24) {
                ...
                if (isset($this->listingid))
                    $sql .= ' WHERE e_id = "' . (int) $this->listingid . '" ';
                elseif (isset($_POST['osrc']))
                    $sql .= " WHERE et_id = '" . $this->input->post('osrc', TRUE) . "' OR  buyerid LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR buyeremail LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR notes LIKE '%" . $this->input->post('osrc', TRUE) . "%' ";
                elseif ($timeframe)
...
''	as	delivery  
  ...
  ";
            if (isset($this->listingid))
                $sql .= ' FROM (warehouse_orders o) LEFT JOIN warehouse w ON o.woid = w.sold_id WHERE w.vended > 0 AND w.deleted = 0 AND w.nr = 0 AND w.channel = 4 AND w.listingid = "' . (int) $this->listingid . '" ';
            elseif (isset($_POST['osrc']))
                $sql .= " FROM (warehouse_orders o) WHERE woid = '" . $this->input->post('osrc', TRUE) . "' OR buyer LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR notes LIKE '%" . $this->input->post('osrc', TRUE) . "%' ";
            elseif ($timeframe)
                $sql .= " FROM (warehouse_orders o)  WHERE timemk <= " . $ofrom . " AND timemk >= " . $oto . " ";
            else
                $sql .= ' FROM  (warehouse_orders o) ';
            //if ($q->num_rows() > 0  && ((isset($this->orderchannel) && $this->orderchannel == 1) || !isset($this->orderchannel))) 
            $sql .= $psql[4];
        }
        if ($channel[2] && ($channel[1] || $channel[4]))
            $sql .= " UNION ALL ";
        if ($channel[2]) {
...
delivery	as	delivery
 ...
 FROM orders ";
            if (isset($this->listingid))
                $sql .= ' WHERE eids LIKE "%|' . (int) $this->listingid . '|%" ';
            elseif (isset($_POST['osrc']))
                $sql .= " WHERE oid = '" . $this->input->post('osrc', TRUE) . "' OR fname LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR lname LIKE '%" . $this->input->post('osrc', TRUE) . "%' OR email LIKE '%" . $this->input->post('osrc', TRUE) . "%' ";
            elseif ($timeframe)
            ...
            foreach ($po->result_array() as $v) {
                ...
                switch ($v['typekey']) {
                    ...
                    case 'website':
                        ...
                        if (isset($v['createdmk'][1]) && isset($v['createdmk'][2]) && isset($v['createdmk'][0]))
                            $v['createdmk'] = (int) mktime(0, 0, 0, $v['createdmk'][1], $v['createdmk'][2], $v['createdmk'][0]);
                        else
                            $v['createdmk'] = false;
                        if (strlen($v['order']) > 9) {
                ...
                if ((int) $v['return_id'] > 0)
                    $ridarray[$v['channel']][$v['return_id']] = $v['return_id'];
            }
        }
        ...
        /*
          $massorders = array();
          if ($po->num_rows() >0) foreach ($po->result_array() as $pov)
          {
          $massorders[$pov['typekey']][$pov['timekey']] = $pov['orderkey'];
          }
          $this->mysmarty->assign('area', 'Transactions');

          $ebsql = 'SELECT * FROM (ebay_transactions t) LEFT JOIN ebay e ON t.e_id = e.e_id';
          $q = 1;
          foreach ($massorders['ebay'] as $k => $v)
          {
          if ($q == 1) $ebsql = ' WHERE et_id = '.$v;
          else  if ($q == 1) $ebsql = ' OR WHERE et_id = '.$v;
          $q++;
          }
          $ebsql .= ' ORDER BY rec DESC';

          $q = $this->db->query($ebsql);
          $mkdtdupcheck = 0;
          ...
          $whsql = 'SELECT * FROM (warehouse_orders)';
          $q = 1;
          foreach ($massorders['warehouse'] as $k => $v)
          {
          if ($q == 1) $whsql = ' WHERE woid = '.$v;
          else  if ($q == 1) $whsql = ' OR WHERE woid = '.$v;
          $q++;
          }
          $whsql .= ' ORDER BY timemk DESC';
          $this->wquery = $this->db->query($whsql);
          unset($idarray);
                ...
                foreach ($this->order['order'] as $k => $ov) {
                    ...
                    if ($f->num_rows() > 0) {
                        ...
                        foreach ($fr as $fl) {
                            ...
                            //printcool ($data);
                            //printcool ($fl);
                            $this->db->update('warehouse', $data, array('wid' => $fl['wid']));
                        }
                    }
                    /* if (isset($ov['sn']))
                      {
    ...
    function OrderComm($oid) {
        ...
        if ($query->num_rows() > 0) {
            ...
            if (isset($_POST['msg'])) {
                ...
                if (strlen($title) < 10)
                    $title = 'Regarding your Order No.' . $oid . ' at ' . $this->config->config['sitename'];
...
<EndFixedPriceItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">';
            ...
            if (isset($xml['EndTime'])) {
                ...
                $this->db->update('ebay', array('ebended' => $ended, 'endedreason' => 'Ended from order ' . (int) $oid, 'sitesell' => 0), array('ebay_id' => (int) $itemid, 'e_id' => (int) $eid));
                ...
                if ($item) {
                    ...
                    if (isset($item->ShippingDetails->ShippingServiceOptions->ShippingServiceCost)) {
                        ...
                        if ($t['ssc'] != (string) $item->ShippingDetails->ShippingServiceOptions->ShippingServiceCost && floater($item->ShippingDetails->ShippingServiceOptions->ShippingServiceCost) > 0) {
                            ...
                            $this->db->update('ebay_transactions', array('ssc' => floater($item->ShippingDetails->ShippingServiceOptions->ShippingServiceCost), 'cascupd' => 2), array('et_id' => $t['et_id']));
                        }
                        ...
                        if ($t['shipping'] != (string) $item->ShippingDetails->ShippingServiceOptions->ShippingService && (string) $item->ShippingDetails->ShippingServiceOptions->ShippingService != '') {
                            ...
                            $this->db->update('ebay_transactions', array('shipping' => (string) $item->ShippingDetails->ShippingServiceOptions->ShippingService, 'cascupd' => 2), array('et_id' => $t['et_id']));
                        }
                    } else
                    ...
                    if (isset($ar['CheckoutStatus'])) {
                        if ($ar['CheckoutStatus'] == 'CustomCode') {
                            if ($t['customcode'] == 0) {
                                ...
                                $this->_logaction('Transactions', 'B', array('customcode' => 0), array('customcode' => 1), 0, $t['itemid'], $t['rec']);
                            }
                            ...
                            if ($t['notpaid'] == 0) {
                                ...
                                $this->_logaction('Transactions', 'B', array('notpaid' => 0), array('notpaid' => 1), 0, $t['itemid'], $t['rec']);
                            }
                            ...
                            if ($t['pendingpay'] == 0) {
                                ...
                                $this->_logaction('Transactions', 'B', array('pendingpay' => 0), array('pendingpay' => 1), 0, $t['itemid'], $t['rec']);
                            }
                            ...
                            if ($t['pendingpay'] == 1) {
                                ...
                                $this->_logaction('Transactions', 'B', array('pendingpay' => $t['pendingpay']), array('pendingpay' => 0), 0, $t['itemid'], $t['rec']);
                            }
                            if ($t['notpaid'] == 1) {
                                echo ' - UPDATED to NotPaid 0';
                                $this->db->update('ebay_transactions', array('notpaid' => 0), array('et_id' => $t['et_id']));
                                $this->_logaction('Transactions', 'B', array('notpaid' => $t['notpaid']), array('notpaid' => 0), 0, $t['itemid'], $t['rec']);
                            }
                            if ($t['customcode'] == 1) {
                                echo ' - UPDATED to CustomCode 0';
                                $this->db->update('ebay_transactions', array('customcode' => 0), array('et_id' => $t['et_id']));
                                $this->_logaction('Transactions', 'B', array('customcode' => $t['customcode']), array('customcode' => 0), 0, $t['itemid'], $t['rec']);
                            }
                        }
                    }
                    unset($ar['paidtime']);
    ...
    function GetShipping() {
        ...
        if ($responseXml) {
            if (!write_file($this->config->config['ebaypath'] . '/shipping.txt', $responseXml))
                GoMail(array('msg_title' => 'Unable to write Shippinh.txt @' . CurrentTime(), 'msg_body' => '', 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
            else
                GoMail(array('msg_title' => 'Shipping written @' . CurrentTime(), 'msg_body' => '', 'msg_date' => CurrentTime()), 'errors@la-tronics.com', $this->config->config['no_reply_email']);
        }
...
';
        ...
        if (isset($store->Store->CustomCategories->CustomCategory)) {
            ...
            if (!$save && isset($sc[(string) $xml->Item->Storefront->StoreCategoryID]))
                echo '<tr><td>eBay Store Category:</td><td>' . $sc[(string) $xml->Item->Storefront->StoreCategoryID] . '</td></tr>';
            elseif (!$save && !isset($sc[(string) $xml->Item->Storefront->StoreCategoryID]))
        ...
        if (!$save)
            ...
            if ($save) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $this->_logaction('UpdateFromEbay', 'M', array('price' => $ebr['price_ch2']), array('price' => (string) $xml->Item->StartPrice), $ebr['e_id'], $data['ebay_id'], 0);
                    ...
                    foreach ($data as $k => $v) {
                        ...
                        if ($k != 'PaymentMethod' && $k != 'shipping' && $k != 'price_ch2' && $k != 'Submitted')
                            $this->_logaction('RelinkFromEbay', $latp, array($k => $olddata), array($k => $v), (int) $ebr['e_id'], $data['ebay_id'], 0);
                    }
...
';
                ...
                if ($query->num_rows() > 0) {
                    ...
                    foreach ($data as $k => $v) {
                        ...
                        if ($k != 'PaymentMethod' && $k != 'shipping' && $k != 'price_ch2')
                            $this->_logaction('RefreshFromEbay', $latp, array($k => $olddata), array($k => $v), (int) $ebr['e_id'], $ebr['ebay_id'], 0);
                    }
    ...
    function UpdateASC($rec = '', $itemid = '') {
        ...
        if ($item) {
            if ((string) $item->ActualShippingCost != $t['asc']) {
                ...
                $this->db->update('ebay_transactions', array('asc' => floater((string) $item->ActualShippingCost)), array('et_id' => $t['et_id']));

                $this->db->select('wid, bcn, ' . $this->Myseller_model_sellingfields());
                $this->db->where('channel', 1);
                $this->db->where('sold_id', $t['et_id']);
                $this->db->where('vended', 1);
                ...
                if ($f->num_rows() > 0) {
                    ...
                    foreach ($fr as $fl) {
                        if ($fl['vended'] == 1)
                            $this->Myseller_model->HandleBCN(array('shipped_actual' => floater((string) $item->ActualShippingCost)), $fl);
                    }
                }

                $this->_logaction('Transactions', 'B', array('ActShipCost' => $t['asc']), array('ActShipCost' => floater((string) $item->ActualShippingCost)), 0, $t['itemid'], $t['rec']);
                $updatedIds .= $t['rec'] . ', ';
                $change++;
            ...
            if ($cascupd == 1)
                $this->db->update('ebay_transactions', array('cascupd' => 1), array('et_id' => $t['et_id']));
            unset($item);
        }
    ...
    function Resetcascupd($id = '') {
        if ((int) $id > 0) {
            $this->db->update('ebay_transactions', array('cascupd' => 0, 'asc' => '0.00'), array('et_id' => $id));
            echo 1;
        }
    }
        ...
        foreach ($datato as $k => $v) {
            if ($v != $datafrom[$k]) {
                ...
                if ($key == '')
                    ...
                    $this->db->insert('ebay_actionlog', array('atype' => $type, 'e_id' => (int) $eid, 'ebay_id' => (int) $itemid, 'time' => CurrentTimeR(), 'datafrom' => $datafrom[$k], 'datato' => $v, 'field' => $k, 'admin' => $admin, 'oid' => (int) $transid, 'okey' => $key, 'ctrl' => $location));
            }
        }
    }

    function _recordsubmiterror($err = array()) {
        //GoMail($err, 'errors@la-tronics.com', $this->config->config['no_reply_email']);
...
//////////////////////////////////////////////	
        ...
        if ($r->num_rows() > 0) {
            ...
            if ($r['nwm'] == 0) {
                ...
                if ($change > 0)
                    $this->db->update('ebay', array('nwm' => 1), array('e_id' => (int) $id));
            }
        } else
...
<g:availability>" . googleavailability($value['qn_ch1']) . "</g:availability>
<g:price>" . $value['price_ch1'] . " USD</g:price>
...
<g:shipping_weight>" . $value['weight_kg'] . " kg</g:shipping_weight>
    ...
    function PopulateItemSpecifics($eid = 0) {
        ...
        if (count($itemspec) > 0 && strlen(serialize($itemspec)) > 10) {
            printcool($itemspec);
            $this->db->update('ebay', array('eBay_specs' => serialize($itemspec)), array('e_id' => (int) $eid));
        }
    }
    ...
    function ApplyItemSpecToActive() {
        ...
        if ($res->num_rows() > 0) {
            foreach ($res->result_array() as $revs) {
                ...
                $this->db->update('ebay', array('housekeeping' => 1), array('e_id' => (int) $revs['e_id']));
