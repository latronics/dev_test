...
$this->db->where("ip_quotes.quote_id", $quote->quote_id);
$quotes_data = $this->db->get("ip_quotes")->result_array();
?>
                ...
                <select name="invoice_group_id" id="invoice_group_id" class="form-control">
                    ...
                    <?php foreach ($invoice_groups as $invoice_group) { ?>
                        <option value="<?php echo $invoice_group->invoice_group_id; ?>"
                                <?php if ($this->mdl_settings->setting('default_invoice_group') == $invoice_group->invoice_group_id) { ?>selected="selected"<?php } else if ($quotes_data[0]['store_name'] == $invoice_group->invoice_group_name) {
                            echo "selected";
                        } ?>>
