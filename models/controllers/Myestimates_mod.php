...
function index()
	{	
		...
		if ($this->query->num_rows() > 0) 
		{
			...
			foreach ($data as $k => $v)
			{
				$v['datemk'] =  str_replace(' ', '', $v['date']);
				$v['datemk'] =  str_replace(':', '', $v['datemk']);
				$v['datemk'] =  str_replace('-', '', $v['datemk']);
		...
		$this->data = $this->Myforms_model->ListItemsNP();
		...
		if ($cquery->num_rows() > 0)
			{
				foreach ($cquery->result_array() as $f)
				{
				$cmatched[$f['f_id']] = TRUE;
				}
			}
		else $cmatched = array();
		krsort($datan);
		...
		$this->mysmarty->assign('creplies', $cmatched);
		$this->mysmarty->assign('requests', $datan);
		$this->mysmarty->assign('reqtype', '');
...
function Estimate()
	{
			$this->mysmarty->assign('reqtype', 'est');
			$this->index();			
	}
function Inquiry()
	{
			$this->mysmarty->assign('reqtype', 'inq');
			$this->index();			
	}	
function Contact()
	{		
			$this->mysmarty->assign('reqtype', 'cont');
			$this->index();			
	}
function Read($cid = '') 
{
