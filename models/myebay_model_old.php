...
function Viewport($view, $page = 1)
{
	...
	switch ($view)
	{
		...
		case "NoEbayspec":
			$specs = 'FROM ebay WHERE `ebay_id` != 0  AND `ebended` IS NULL AND `eBay_specs` IS NULL';
...
function ListItems($string, $where = '', $ended = '', $zero = '', $mm = '', $bcnmm, $sitesell, $page = null, $category_id = null)
	{

		if ((int)$page > 0) $page--;
		$this->db->select("ebay.e_id, ebay.dbrel, ebay.e_title, ebay.e_part, ebay.e_qpart, ebay.old_e_part, ebay.admin_id, ebay.storeCatTitle, ebay.pCTitle, ebay.listingDuration, ebay.startPrice,
		 ebay.buyItNowPrice,  ebay.quantity, ebay.ebayquantity, ebay.ebay_submitted, ebay.e_img1, ebay.idpath, ebay.link, ebay.ebay_id, ebay.submitlog, ebay.gsid1, ebay.gsid2, ebay.gsid3, ebay.gsid4,
		  ebay.gsid5, ebay.autorev, ebay.autorevtxt, ebay.ebsold, ebay.sitesell, ebay.nwm, ebay.ebended, ebay.endedreason, ebay.e_ebayq, ebay.ngen, ebay.price_ch1, ebay.price_ch2, ebay.price_ch3, ebay.qn_ch1,
		   ebay.qn_ch2, ebay.qn_ch3, ebay.audit, ebay.ostock, ebay.ooskeepalive,ebay.lock_google_cat, ebay.lock_ebay_cat, ebay.lock_amazon_cat, wsc_title");
		$this->db->join("warehouse_sku_categories", "warehouse_sku_categories.wsc_id = ebay.storeCatID");
		if ($where == 1) $wl = 'e_part';
		elseif ($where == 2) $wl = 'ebay_id';
		elseif ($where == 3) $wl = 'e_id';
		elseif ($where == 4) $wl = 'storeCatID';
