...
This sample retrieves recommended Item Specifics for a category on the eBay US site. If recommendations are available for the category, each NameRecommendation node returns the recommended name, validation rules, and recommended values (if any). (Categories that support custom Item Specifics do not necessarily have recommendations. So, this call does not always return data.)
		...
		*/
		...
		//GET EXISTISTING DB CATS
        $itsp = $this->db->get("item_specifics")->result_array();
        foreach ($itsp as $i)
        {
            $isarr[$i['is_primcat']][$i['is_name']] = $i;
        }
        $itspv = $this->db->get("item_specifics_values")->result_array();
        foreach ($itspv as $iv)
        {
            $isvarr[$iv['is_primcat']][$iv['is_id']][$iv['isv_value']] = $iv;
        }
		...
		foreach (glob($dir."*.xml") as $file) {
			...
			foreach ($allcats->Recommendations as $rc)
			{
				...
				if (isset($usablecats[(int)$rc->CategoryID]))
				{
                    ...
                    {
                        $vals = array(
                            'is_primcat' => (int)$rc->CategoryID,
                            'is_name' => (string)$nr->Name,
                            'is_maxvalues' => (string)$nr->ValidationRules->MaxValues,
                            'is_selectionmode' => (string)$nr->ValidationRules->SelectionMode,
                            'is_variationspecifics' => (string)$nr->ValidationRules->VariationSpecifics
                        );
                        if (isset($isarr[(int)$rc->CategoryID][(string)$nr->Name])) {
                            $upd = false;

                            if ($isarr[(int)$rc->CategoryID][(string)$nr->Name]['is_maxvalues'] != (string)$nr->ValidationRules->MaxValues) $upd = true;
                            if ($isarr[(int)$rc->CategoryID][(string)$nr->Name]['is_selectionmode'] != (string)$nr->ValidationRules->SelectionMode) $upd = true;
                            if ($isarr[(int)$rc->CategoryID][(string)$nr->Name]['is_variationspecifics'] != (string)$nr->ValidationRules->VariationSpecifics) $upd = true;
                            if ($upd) {
                                $this->db->update('item_specifics', $vals, array('is_id' => $isarr[(int)$rc->CategoryID][(string)$nr->Name]['is_id']));
                                $is_id = $isarr[(int)$rc->CategoryID][(string)$nr->Name]['is_id'];
                            }
                        } else {
                            $this->db->insert('item_specifics', $vals);
                            $is_id = $this->db->insert_id();
                        }

                        foreach($nr->ValueRecommendation as $vr)
                        {
                            if (isset($isvarr[(int)$rc->CategoryID][$is_id])) {

                                if ($isvarr[(int)$rc->CategoryID][$is_id][(string)$vr->Value]['isv_value'] != (string)$vr->Value)
                                {
                                    $this->db->update('item_specifics_values', array('isv_value' => (string)$vr->Value), array('isv_id' => $isvarr[(int)$rc->CategoryID][$is_id]['isv_id']));
                                }
                            } else {

                                $this->db->insert('item_specifics_values', array('is_id' => $is_id, 'is_primcat' => (int)$rc->CategoryID, 'isv_value' => (string)$vr->Value));
                            }
                        }
                    }
