...
class Mdl_Reports extends CI_Model {
    ...
    public function sales_by_client($from_date = null, $to_date = null, $client_name = null) {

        $this->db->select('distinct(ip_clients.client_name), ip_invoices.invoice_id, ip_clients.client_id');
        ...
        if ($from_date and $to_date) {
            ...
            $this->db->where('ip_clients.client_name <>', '');
            $this->db->where("ip_invoices.invoice_date_created >=' $from_date' and ip_invoices.invoice_date_created <= '$to_date'");
            $this->db->like('ip_clients.client_name', $client_name);
        } else {
            $this->db->join("ip_invoices", "ip_invoices.client_id = ip_clients.client_id");
    ...
    public function payment_history($from_date = null, $to_date = null, $store = null, $products = null) {
        ...
        if ($store != 1) {
            $this->mdl_payments->where('ip_payments.store', $store);
        }
        if (($products != null)&& ($products != 'all')) {
            $this->mdl_payments->where("ip_invoice_items.item_product_id",$products);
        }
