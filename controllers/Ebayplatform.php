					...
					/*						A - BCN
...

                        {
                           ...
                           // echo '<br>ItemID: '.$v['data']['TransactionIdentity']['ItemId'];
							...
							$this->_notifyM('Return Notif: '.$v['type'],$v['data']);
							 ...
							 else $this->db->update('ebay_transactions',
								 array('returnnotif' => $v['type'],
									 'returnurl' => $v['data']['ReturnLink'],
									 'returnid'=>(int)$v['data']['ReturnId']),
								 array('transid' => $v['data']['TransactionIdentity']['TransactionId'],'itemid'=>$v['data']['TransactionIdentity']['ItemId']));
							 
								$this->_notifyM('Return Notif 2: '.$v['type'], $details);
                            /*
...
(
    ...
    [ReturnTrackingId] => 9302020130300854200554
...

                        {
                           ...
                           // echo '<br>TransactionID: '.$v['data']['TransactionIdentity']['TransactionId'];
							$this->_notifyM('Return Notif: '.$v['type'],$v['data']);
							$this->_GetReturnData((int)$v['data']['ReturnId'],$v['type']);
							/*GoMail(array(
...
(
    ...
    [ReturnTrackingId] => 9382120128700436969721
			...
			'msg_date' => CurrentTime()			
							...
							else $this->db->update('ebay_transactions', array('returnnotif' => $v['type']), array('transid' => $v['data']['TransactionIdentity']['TransactionId'],'itemid'=>$v['data']['TransactionIdentity']['ItemId']));
                               // echo '<br>Found Return Created';
...
function CheckRefundStatus($etid = 0)
{
	$this->load->model('Auth_model');
	$this->Auth_model->VerifyAdmin();

	$this->db->select('returnId');
	$this->db->where('et_id',(int)$etid);
	$ebtr = $this->db->get('ebay_transactions');
	if ($ebtr->num_rows() > 0)
	{
		$ebdata=$ebtr->row_array();
		if ($ebdata['returnId'] > 0)
		{
			$data = $this->_GetReturnData($ebdata['returnId']);
			printcool($data);
		}
		else printcool ('ReturnId is 0');
	}
	...
	function _GetReturnData($returnid = 0, $txt = '')// = 5046058098
	{
		...
		$insert['ebaystatus'] = ucwords(strtolower(str_replace('_', ' ', $data['summary']['status'])));
		...
		$this->db->where('transid',$insert['transactionid']);
		$ebtr = $this->db->get('ebay_transactions');
		if ($ebtr->num_rows() > 0)
		{
			$ebdata=$ebtr->row_array();
			$insert['et_id']= $ebdata['et_id'];

			if ($insert['state'] == 'closed')
			{
				if (floater($ebdata['paid']) > 0 && $ebdata['sellingstatus'] != 'PartiallyPaid') $this->db->update('ebay_transactions', array('paid' => 0,'sellingstatus'=> 'Refunded'),array('et_id' => $ebdata['et_id']));
			}
		}

		$this->db->select('er_id');
		$this->db->where('transactionid',$insert['transactionid']);
		$e = $this->db->get('ebay_refunds');
		if ($e->num_rows() > 0)
		{
			$er_id = $e->row_array();
			$this->db->update('ebay_refunds', $insert,array('er_id' => $er_id['er_id']));
		}
		else $this->db->insert('ebay_refunds', $insert);

		if (isset($insert['actualRefund'])) $answer['ebayRefundAmount'] = $insert['actualRefund'];
		elseif (isset($insert['estimatedAmount']))  $answer['ebayRefundAmount'] = $insert['estimatedAmount'];
		...
		if (isset($insert['actualRefund'])) $answer['ebayRefundAmount'] = $insert['actualRefund'];
		else $answer['ebayRefundAmount'] = $data['summary']['sellerTotalRefund']['estimatedRefundAmount']['value'];


		$answer['ebayreturntime'] = CleanBadDate($data['summary']['creationInfo']['creationDate']['value']);
		if (isset($insert['shippingLabelCost']))
		{
			if(strtolower($insert['returntype']) == 'remorse') $answer['ebayreturnshipment'] = 0;
			else $answer['ebayreturnshipment'] = floater($insert['shippingLabelCost']);
		}
		else
		{
