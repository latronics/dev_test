...
class Myebay extends Controller
{
    ...
    function index()
    {
        ...
        $this->ListItems($page = 1, $qtt_show = 30, $searched = false, $category_id = '0');
    }
    function loadcat()
    {

        $this->db->where("wsc_parent", 0);
        $cat_data_dad = $this->db->get("warehouse_sku_categories")->result_object();

        foreach ($cat_data_dad as $categories) {

                $categories_json[] = array(
                    "id" => "$categories->wsc_id",
                    "parent" => "#",
                    "text" => "$categories->wsc_title"
                );

                $this->db->where("wsc_parent", $categories->wsc_id);
                $cat_data_child = $this->db->get("warehouse_sku_categories")->result_object();
            foreach($cat_data_child as $cat_data_child)
            {
                $this->db->where("storeCatID", $cat_data_child->wsc_id);
                $num_prod = $this->db->get("ebay")->num_rows();
                $categories_json[] = array(
                    "id" => "$cat_data_child->wsc_id",
                    "parent" => "$cat_data_child->wsc_parent",
                    "text" => "$cat_data_child->wsc_title(<b style='color:green;'>$num_prod</b>)"
                );
        ...
        return json_encode($categories_json);
    }
    function ListItems($qtt_show = 30, $searched = false, $category_id = '0', $page = 1)
    {
        $qtt_show = 30;
        ...
        $data = $this->Myebay_model->ListItems($string, $where, $ended, $zero, $mm, $bcnmm, $sitesell, $page, $category_id);
        ...
        */
        ...
        $this->mysmarty->assign('pages', $data['pages']);
        $this->mysmarty->assign('page', (int) $page);
        $this->mysmarty->assign('cost_validator', $this->session->userdata('listings'));
        ...
        if($category_id != '0')
        {
            $this->db->where("storeCatID", $category_id);
            $data['count']['all'] = $this->db->get("ebay")->num_rows();
        }
        //CONFIG PAGINATION
        ...
        if($category_id != '0')
        {
            ...
            $config['base_url'] = base_url() . 'Myebay/ListItems/'.$qtt_show.'/false/'.$category_id;
        }else {
        ...
        if(isset($string))
        {
        $this->db->like("StoreCatID", $category_id);
        $data['count']['all'] = $this->db->get("ebay")->num_rows();
        }
        ...
        $this->mysmarty->assign("total_pages", round($data['count']['all'] / $qtt_show + 1));
        $this->mysmarty->assign('pagination', $this->pagination->create_links());
        $show_categories = 1;
        $this->mysmarty->assign("show_categories", $show_categories);
...
-->
	...
	function ReviseActiveDescription()
	{
		$this->db->select('e_id');
		$this->db->where("ebended", NULL);
		$this->db->where('ebay_id !=', 0);
		$this->db->where('housekeeping', 0);
		$this->db->orderby('e_id', 'DESC');
		$listings = $this->db->get('ebay')->result_array();
		require_once($this->config->config['ebaypath'].'get-common/keys.php');
		require_once($this->config->config['ebaypath'].'get-common/eBaySession.php');

		foreach($listings as $l)
		{
			$this->ReviseEbayDescription($l['e_id']);
			$this->db->update('ebay', array('housekeeping' => 1),array('e_id' => $l['e_id']));
			printcool($l);
		}
    ...
    function Edit($itemid = '', $catID = 0, $merge = false, $called = false)
    {
        ...
        if ($this->id > 0)
        {
            ...
            */
            ...
            $this->db->where('is_primcat', $this->displays['primaryCategory']);
            $itsp = $this->db->get("item_specifics")->result_array();
            $isarr = array();
            foreach ($itsp as $i)
            {
                $isarr[$i['is_name']] = $i;
            ...
            $this->db->where('is_primcat', $this->displays['primaryCategory']);
            $this->db->orderby('isv_value','ASC');
            $itspv = $this->db->get("item_specifics_values")->result_array();
            $isvarr = array();
            foreach ($itspv as $iv)
            {
                $isvarr[$iv['is_id']][$iv['isv_value']] = $iv;
            }
            
            $this->mysmarty->assign('item_specifics', $isarr);
            $this->mysmarty->assign('item_specifics_values', $isvarr);
            ...
            if (!$merge)
            {
                ...
                //$this->form_validation->set_rules('store_categories', 'Store Category', 'trim|required|xss_clean');
            }
...
');
    ...
    function orderssearchnew($page = 1, $perpage = 100, $filtertype = false, $filtersubtype = false)
    {
        $this->load->model('Myorders_model');
...
<g:shipping_weight>" . $value['weight_kg'] . " kg</g:shipping_weight>
	...
	function SetFraud($etid = 0)
	{
		if ((int)$etid >0)
		{
			$this->db->select('et_id, paid');
			$this->db->where('et_id',(int)$etid);
			...
			if($ebtr->num_rows() > 0)
			{
				...
				{
					$update['paid']=0;
					$update['sellingstatus']='Fraud $'.$ebdata['paid'].' (Set by '.$this->session->userdata['ownnames'].') @ '.CurrentTime();
					$this->db->update('ebay_transactions',$update,array('et_id'=>(int)$ebdata['et_id']));
					echo 'Set: '.$update['sellingstatus'];
				}
			}
