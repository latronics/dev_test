	...
	/////
		...
		if ($this->form_validation->run() == FALSE)
			...
			{	
				...
				if ($client['country'] == 'United States of America') $courier = $refreshedcart['shipping']['domestic'];
				else $courier = $refreshedcart['shipping']['international'];
...
function _ProcessCheckOut()	
	{
		...
		if ($this->insertdata['endprice'] > 0) {
				...
				if (!isset($this->session->userdata['user_id'])) 
					{									
						$this->_RegisterNewCustomer('', $this->insertdata);	
