...
class Mdl_Invoices extends Model {
    ...
    public function default_select() {
        ...
        $admin365->select("
			...
			ip_invoices.*", false);
        if ($this->store_id() != 1) {
            $admin365->where("ip_invoices.store", $this->store_id());
        }
    }

    public function default_order_by() {
        $admin365 = $this->load->database("365admin", true);
        $admin365->order_by('ip_invoices.invoice_id DESC');
    }

    public function default_join() {
        $admin365 = $this->load->database("365admin", true);
        $admin365->join('ip_clients', 'ip_clients.client_id = ip_invoices.client_id');
        $admin365->join('ip_users', 'ip_users.user_id = ip_invoices.user_id');
        $admin365->join('ip_invoice_amounts', 'ip_invoice_amounts.invoice_id = ip_invoices.invoice_id', 'left');
        $admin365->join('ip_client_custom', 'ip_client_custom.client_id = ip_clients.client_id', 'left');
        $admin365->join('ip_user_custom', 'ip_user_custom.user_id = ip_users.user_id', 'left');
        $admin365->join('ip_invoice_custom', 'ip_invoice_custom.invoice_id = ip_invoices.invoice_id', 'left');
    }
    ...
    public function create($store = null, $db_array = null, $include_invoice_tax_rates = true) {
        $admin365 = $this->load->database("365admin", true);
        ...
        if ($store != 1) {
            ...
            $admin365->update("ip_invoices", $store_array);
            $admin365->insert('ip_invoice_amounts', $db_array);
            ...
            if ($include_invoice_tax_rates) {
                ...
                if ($this->mdl_settings->setting('default_invoice_tax_rate')) {
                    ...
                    $admin365->insert('ip_invoice_tax_rates', $db_array);
                }
            }
    ...
    public function mark_viewed($invoice_id) {
        $admin365 = $this->load->database("365admin", true);
        $admin365->select('invoice_status_id');
        $admin365->where('invoice_id', $invoice_id);

        $invoice = $admin365->get('ip_invoices');

        if ($invoice->num_rows()) {
            if ($invoice->row()->invoice_status_id == 2) {
                $admin365->where('invoice_id', $invoice_id);
                $admin365->set('invoice_status_id', 3);
    ...
    public function mark_sent($invoice_id) {
        ...
        if ($invoice->num_rows()) {
            if ($invoice->row()->invoice_status_id == 1) {
                $admin365->where('invoice_id', $invoice_id);
                $admin365->set('invoice_status_id', 2);
                $admin365->update('ip_invoices');
            }
